...
│ 2. DATA LAYER        сессии, ATR, RVOL, Δ-источник, LTF+FP мост   │
├──────────────────────────────────────────────────────────────────┤
│ 2b. L3 FOOTPRINT     изолированный регион, единая точка входа    │  ← removable
...
> **Исправлено в P11.** Таблица ниже раньше описывала замысел, а не код. Фактически
> `Performance Mode` управляет **четырьмя** параметрами. Модель распределения,
> источник данных и то, что рисуется, — это **отдельные переключатели**, и именно
> они определяют основную нагрузку. Измерения — в `docs/P11-acceptance.md`.

### Что режим действительно ограничивает

| Параметр | **Lite** | **Standard** | **Pro** |
|---|---|---|---|
| Бинов в профиле | `min(Bins, 24)` | `min(Bins, 150)` | `Bins` |
| Профилей на графике | 1 | `min(Profiles, 2)` | `Profiles` |
| Активных зон | `min(Max zones, 10)` | `min(Max zones, 25)` | `Max zones` |
| Пересчёт POC/VA (логика) | каждые 10 баров | каждые `max(N, 5)` | каждые `max(N, 1)` |
| Developing-линии | выключены | включены | включены |

### Чем режим НЕ управляет (отдельные переключатели)

| Параметр | Где настраивается | Влияние на нагрузку |
|---|---|---|
| Модель распределения (M1/M2/M3) | `Bar distribution model` | **определяет точность**, стоимость мала |
| Источник данных (L0–L3) | `Profile data source` | **самая дорогая подсистема**: L2 LTF ≈ 250–350 операций/бар |
| Гистограмма профиля | `Draw profile histogram` | боксы |
| HVN / LVN | `Show HVN / LVN` | только на последнем баре |
| TPO-профиль | `Show TPO profile` | — |
| Модель направленного объёма | `Directional volume model` | L2/L3 |

**Практическое следствие:** выбор `Lite` **не** отключает LTF. Если нужна
минимальная нагрузка — ставьте `Profile data source = L1` вместе с режимом Lite.

### Измеренная стоимость профильного движка (симуляция, `tools/profile_sim.py`)

| Конфигурация | Операций/бар | Ошибка POC (тиков) |
|---|---|---|
| Lite (24 бина, M3, раз в 10 баров) | 12 | 0.79 |
| Standard (50 бинов, M3, раз в 5 баров) | 30 | 0.54 |
| Pro при `Bins = 50` | 110 | 0.54 |
| Pro при `Bins = 150` | 318 | 0.50 |

Режим — единственный input, меняющий дефолты сразу нескольких подсистем;
все отдельные переключатели остаются доступны и перекрывают его.
...
## 3.8. Поправка после проверки `request.footprint()` (см. `docs/08-footprint.md`)

**Источники данных профиля — 4 уровня, выбор через один input `Profile data source`:**

| Уровень | Источник | Доступность |
|---|---|---|
| `L0 BAR (close)` | M1 | все планы |
| `L1 BAR (distributed)` | M2 / M3 | все планы |
| `L2 LTF` | `request.security_lower_tf()` | все планы, глубина ограничена |
| `L3 FOOTPRINT` | `request.footprint()` | **Premium / Ultimate** |

Режим `Auto` пробует L3 → L2 → L1 и фиксирует фактически применённый уровень в поле `Profile.model`.
Дашборд обязан показывать **фактический**, а не запрошенный уровень.

**Изоляция L3.** Весь код, обращающийся к `request.footprint()`, заключён в регион
`//#region L3 FOOTPRINT ADAPTER … //#endregion` с единственной точкой входа
`fpBarContribution(...)`. Удаление региона целиком даёт валидный билд для всех тарифов
(вариант B из `08-footprint.md` §5). Ни один другой модуль не обращается к типам
`footprint` / `volume_row` напрямую.

**Один запрос.** `request.footprint()` может встречаться в скрипте **ровно один раз**,
поэтому `ticks_per_row` — глобальный параметр, и footprint запрашивается только для
текущего таймфрейма (агрегацию по сессии делает наш Profile Engine).

**Вес Volume-компонента** в scoring: `0.5` при L0–L2, `1.0` при активном L3 —
потому что при L3 компонент опирается на реальные данные, а не на прокси.
